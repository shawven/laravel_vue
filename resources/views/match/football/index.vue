<template>
    <div>
        <list-table ref="listTable" :url="url" :columns="columns" title="竞彩足球列表" :sort="[{date: 'desc'}]"
            ...
            <div  slot="button-block">
                <auth-button name="match_football_create" @click="creatorModal = true" />
                <auth-button name="match_football_update" @click="editorModal = selectOne()"/>
            </div>
            <div>
                <editor :url="url" :edit="false" v-model="creatorModal" title="新增竞彩篮球"  @created="created"/>
            </div>
            <div slot="select-item-block" slot-scope="{item}">
                <editor :url="url" :item="item" v-model="editorModal" title="编辑竞彩篮球" @updated="updated"/>
            </div>
        </list-table>
    </div>
</template>
    ...
    export default {
        ...
        data() {
            let searchObj = {
                ...
                endtime: '',
                deadline: '',
                j: {fbo: 'id,rq,nspf,spf,bf,jqs,bqc'},
                jo: {fbo: 'l,match_id,id'},
                ja: {fbo: 'odds'}
            };
            return {
                url: '/api/matches/football',
                creatorModal: false,
                editorModal: false,
