    ...
    <div style="display:inline-block;padding:0 6px;">
        <Dropdown trigger="click" @on-click="setTheme">
            <a href="javascript:void(0)">
                <Icon :style="{marginTop: '-2px', verticalAlign: 'middle'}" color="#495060" :size="18" type="ios-options"></Icon>
...
export default {
    ...
    methods: {
        setTheme (themeFile) {
            ...
            if (config.env.indexOf('dev') > -1) {
                stylePath = './resources/components/theme-switch/theme/';
            } else {
                stylePath = 'dist/';
            }
            if (mainTheme !== 'b') {
    ...
    created () {
        let path = '';
        if (config.env.indexOf('dev') > -1) {
            path = './resources/components/theme-switch/theme/';
        } else {
            path = 'dist/';
        }
